...
const Products = ({ showModal }) => {
  const phrases = ["Check our products.", "More information coming."];
  ...
  if (showModal) {
    return null; // Hide the Products component when showModal is true
  }

  return (
  <ProductsContainer onClick={handleProductsClick} isClicked={isClicked} showModal={showModal}>
...
const ProductsContainer = styled.div`
  background-color: ${({ isClicked, isHeaderHovered }) => (isClicked || isHeaderHovered ? 'green' : 'darkcyan')};
  padding: 20px;
  text-align: center;
  height: 100%;
  border: 1px solid;
