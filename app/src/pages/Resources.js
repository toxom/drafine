...
const Resources = ({ showModal }) => {
  const phrases = ["Check our Resources.", "More information coming."];
  ...
  if (showModal) {
    return null; // Hide the Resources component when showModal is true
  }

  return (
  <ResourcesContainer onClick={handleResourcesClick} isClicked={isClicked} showModal={showModal}>
...
const ResourcesContainer = styled.div`
  background-color: ${({ isClicked, isHeaderHovered }) => (isClicked || isHeaderHovered ? 'green' : 'pink')};
  padding: 20px;
  text-align: center;
  height: 100%;
  border: 1px solid;
